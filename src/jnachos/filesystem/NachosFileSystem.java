...
public class NachosFileSystem implements FileSystem {
	...
	private NachosOpenFile mFreeFragmentMapFile;
	/**
	...
	public NachosFileSystem(boolean pFormat) {
		...
		if (pFormat) {
			...
			freeFragmentMap.mark(DirectorySector * FRAGMENTNUM);
			freeFragmentMap.mark((DirectorySector * FRAGMENTNUM) + 1);
			
			
			//freeMap.mark(FragmentSector);
			freeFragmentMap.mark(FragmentSector * FRAGMENTNUM);
			freeFragmentMap.mark((FragmentSector * FRAGMENTNUM) + 1);
	...
	public boolean create(String pName, int pInitialSize) {
		...
		if (directory.find(pName) != -1) {
			...
			//sector = freeMap.find();
			sector = freeFragmentMap.findFirstFreeEvenSector();
			
			
			if (sector == -1) {
				success = false; // no free block for file header
			} else if (!directory.add(pName, sector/NachosFileSystem.FRAGMENTNUM)) {
				...
				if (!hdr.allocateFragment(freeFragmentMap, pInitialSize)) {
					...
					hdr.writeBack(sector/NachosFileSystem.FRAGMENTNUM);
					directory.writeBack(mDirectoryFile);
	...
	/**
	 * Function to expand the file if the new data is more than the available space
	 * */
	public boolean expandFile(int Size,int fragmentToFree,FileHeader hdr,int sector)
	{
		BitMap freeFragmentMap = new BitMap(Disk.NumSectors * NachosFileSystem.FRAGMENTNUM); // assuming a sector is divided into two fragments

		freeFragmentMap = new BitMap(Disk.NumSectors * NachosFileSystem.FRAGMENTNUM);
		freeFragmentMap.fetchFrom(mFreeFragmentMapFile);
		if(fragmentToFree!=-1)
		{
			freeFragmentMap.clear(hdr.byteToFragment(fragmentToFree));
			hdr.deallocateFragments(freeFragmentMap,fragmentToFree);
		}
		...
		if (!hdr.allocateFragment(freeFragmentMap, Size)) {
			success = false; // no space on disk for data
		} else {
			success = true;
			// everything worked, flush all changes back to disk
		...
		// find a sector to hold the file header
		//sector = freeMap.find();
		//int sector = freeFragmentMap.findFirstFreeEvenSector();
