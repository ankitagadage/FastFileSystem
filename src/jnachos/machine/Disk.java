...
public class Disk implements VoidFunctionPtr {
	...
	public static final int FragmentSize = SectorSize / NachosFileSystem.FRAGMENTNUM;
	 ...
	 *            buffer to hold the incoming bytes.
	 */
	public void readFragmentRequest(int fragmentNumber, byte[] data) {
		int ticks = computeLatency(fragmentNumber/(SectorSize/FragmentSize), false);

		assert (!mActive);
		// only one request at a time
		assert ((fragmentNumber >= 0) && (fragmentNumber < NumSectors*(SectorSize/FragmentSize)));

		Debug.print('d', "Reading from sector" + fragmentNumber);

		JavaSys.lseek(mFileno, (SectorSize/(SectorSize/FragmentSize)) * fragmentNumber + MagicSize);
		JavaSys.read(mFileno, data, SectorSize/(SectorSize/FragmentSize));

		if (Debug.isEnabled('d')) {
			printSector(false, fragmentNumber, data);
		}

		mActive = true;
		updateLast(fragmentNumber);
	...
	public void writeFragmentRequest(int FragmentNumber, byte[] data) {
		...
		Debug.print('d', "Location: " + SectorSize/(SectorSize/FragmentSize) * FragmentNumber + MagicSize);
		JavaSys.lseek(mFileno, FragmentSize * FragmentNumber + MagicSize);
		JavaSys.writeFile(mFileno, data, SectorSize/(SectorSize/FragmentSize));
		...
		mActive = true;
		updateLast(FragmentNumber/(SectorSize/FragmentSize));
