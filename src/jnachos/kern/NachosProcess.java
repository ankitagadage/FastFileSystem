...
public class NachosProcess implements Runnable {
	...
	public static synchronized int getmProcessCount() {
		return mProcessCount;
	}

	public static synchronized void setmProcessCount(int mProcessCount) {
		NachosProcess.mProcessCount = mProcessCount;
	}
	...
	public int getmProcessId() {
		return mProcessId;
	}

	public void setmProcessId(int mProcessId) {
	...
	public NachosProcess(String pProcessName) {
		...
		if(mProcessCount<MAX_PROCESS_COUNT)
			mProcessId = mProcessCount++	;
		else
		{
			mProcessCount = 1;
			mProcessId = mProcessCount++	;
		...
		JNachos.getProcessIdList().add(mProcessId);
	...
	public void Exec(String filename)
	{
		...
		JNachos.getCurrentProcess().setSpace(new AddrSpace(executable));
	...
	public synchronized void switchProcess(NachosProcess pNextProcess) {
		...
		//System.out.println( "Switching from process " + oldProcess.getName() + " to process " + pNextProcess.getName());
	...
	public void saveUserState(int num,int val) {
		
			//mUserRegisters[num] = val;
			Machine.writeRegister(num,val);
